...
<div class="container " >
    <div class="row justify-content-center my-0 py-0 m-3">
        <div class="col-6   " >
            <h2>Salmos de:</h2>
            <ul class="list-group list-group-numbered py-0 px-4 align-items-baseline">
                @foreach ($intenciones as $intencion)
                <a href="lectura/{{$intencion->id}}/{{$intencion->name}}" class="list-group-item list-group-item-action border-0 fs-5" id="enlace-lectura">
                    {{ $intencion->name }}
                   <br> <span class="fs-6 py-0"> Capitulos   {{ $capitulos[$intencion->id] }} </span>
                  </a>
